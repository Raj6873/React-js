...
export default function AdminDashboard() {
  ...
  return (
    <div className={`min-h-screen flex transition-colors duration-300
      ...
      {/* Sidebar */}
      <aside className={`w-80 p-6 flex flex-col shadow-2xl transition-colors duration-300
        ${theme === 'light' 
            ...
            {[
                      ...
                      <div className="space-y-2 max-h-[200px] overflow-y-auto custom-scrollbar pr-2">
                        {tasks
                          ...
                          .map((task) => (
                            <div
                              key={task.id}
                              className={`p-3 rounded-lg text-sm bg-white/10 hover:bg-white/20 transition-all duration-300 
                                ${task.id === lastAddedId ? 'ring-2 ring-indigo-400 animate-pulse' : ''}`}
                            >
              ...
              <li key={item.text}>
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-4 py-2 rounded-lg transition text-lg
      ...
      <div className="flex-1 flex flex-col">
        {/* Header */}
  <header className={`flex items-center justify-between px-10 py-4 sticky top-0 z-10 transition-colors duration-300
            ${theme === 'light' 
          ...
          <div className="flex items-center gap-4">
            ...
            <span className={`font-medium transition-colors duration-300
              ${theme === 'light' ? 'text-indigo-600' : 'text-gray-300'}`}>
              Welcome, Admin
            </span>
            <img
              src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTciKR9jiQOAGxhyDrVTfEzZXii4LKBE8kyAA&s"
              alt="User"
              className={`h-11 w-11 rounded-full shadow-sm transition-colors duration-300 
                ${theme === 'light' 
        ...
        <main className="flex-1 overflow-y-auto p-10">
          <div className="max-w-5xl mx-auto">
            <h1 className={`text-5xl font-extrabold mb-12 text-center drop-shadow transition-colors duration-300
              ${theme === 'light' ? 'text-indigo-900' : 'text-gray-100'}`}>
              🚀 To-Do Management
            </h1>

            {/* Task Form */}
            <div className={`p-8 rounded-2xl shadow-xl max-w-lg mx-auto mb-14 transition-colors duration-300
              ${theme === 'light' 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
              <div className={`rounded-2xl shadow-lg p-6 transition-all duration-300 hover:shadow-2xl
                ${theme === 'light'
      ...
      <div className="fixed top-6 right-6 z-50 flex flex-col gap-3">
        {toasts.map((t) => (
          <div
            key={t.id}
            className={`relative flex items-center gap-3 p-3 rounded-lg shadow-lg border transition-colors duration-300 ${
              theme === 'light'
                ? t.type === "success"
      ...
      {/* Apply dark mode styles to scrollbars */}
      <style>{`
        ${theme === 'dark' ? `
          ::-webkit-scrollbar {
            width: 8px;
