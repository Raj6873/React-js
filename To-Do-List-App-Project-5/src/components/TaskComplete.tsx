...
export default function TaskPending({
  ...
  return (
    <div className="p-8 rounded-3xl shadow-2xl w-full max-w-lg h-96 bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 overflow-hidden space-y-6 border border-indigo-100 flex flex-col">
      {/* Embedded style for custom scrollbar - generally not recommended for production */}
      <style>{`
        .custom-scrollbar::-webkit-scrollbar {
          width: 8px; /* Width of the scrollbar */
        }
        .custom-scrollbar::-webkit-scrollbar-track {
          background: #f1f5f9; /* Light background for the track */
          border-radius: 10px;
        }
        .custom-scrollbar::-webkit-scrollbar-thumb {
          background: #d1d5db; /* Grey thumb */
          border-radius: 10px;
        }
        .custom-scrollbar::-webkit-scrollbar-thumb:hover {
          background: #a1a1aa; /* Darker grey on hover */
        }
        /* For Firefox */
        .custom-scrollbar {
          scrollbar-width: thin; /* "auto" or "thin" */
          scrollbar-color: #d1d5db #f1f5f9; /* thumb and track color */
        }
      `}</style>
